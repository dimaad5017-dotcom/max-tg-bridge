...
| Удаление сообщения                  | пометкой       | да, реакцией 💩 |
...
- `MAX_SHOW_ONLINE` — показывать ли собеседникам, что ты в сети. Пусто — значит
  не показывать, и это правильный выбор: мост на связи круглые сутки, так что
  «да» здесь означает «всегда онлайн». Подробнее — [«Про «вечно в
  сети»»](#про-вечно-в-сети).
...
- **💩 под сообщением** → стереть его и в Telegram, и в MAX у всех. Эта реакция
  ...
  **Telegram не сообщает ботам об удалении сообщений вообще**. Такого события
  ...
  реакция, кроме 👀. И учти чужое ограничение: **копию в Telegram бот вправе
  удалить только в первые 48 часов** — из MAX сообщение уйдёт в любом случае, а
  старую копию здесь придётся убрать руками, мост про это скажет.
...
| `/hidden on` / `off`        | прятать ли своё «был в сети» в MAX               |
...
**Мост так больше не делает.** Признак снимается при запуске, и собеседники
видят обычное «был в сети» с последнего твоего захода. Если по какой-то причине
онлайн нужен — строка `MAX_SHOW_ONLINE=да` в `.env`.

`/hidden on` — про другое и работает поверх: он прячет «был в сети» вообще, это
настройка приватности самого MAX, та же, что в его приложении. Разница простая:
`MAX_SHOW_ONLINE` решает, врать ли про твоё присутствие, а `/hidden` — показывать
ли его кому-то вообще.
