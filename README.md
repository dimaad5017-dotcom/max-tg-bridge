...
Двойной клик по **`0-установка.cmd`**. Он проверит Python, создаст отдельное
...
Двойной клик по **`2-вход-в-MAX.cmd`**.
...
Двойной клик по **`3-запустить-мост.cmd`**.
...
Перетащи туда `3-запустить-мост.cmd` **правой** кнопкой и выбери
...
```
max-tg-bridge/
├── 0-установка.cmd            создаёт окружение и ставит библиотеки
├── 1-настройки.cmd            открывает .env в блокноте
├── 2-вход-в-MAX.cmd           разовый вход по SMS
├── 3-запустить-мост.cmd       запуск, сам поднимается после падения
...
└── cache/                     создаётся при первом входе, в git не попадает
...
`0-установка.cmd` и `3-запустить-мост.cmd`, они этой проблемы не касаются.
Если всё же нужен PowerShell:
`Set-ExecutionPolicy -Scope CurrentUser RemoteSigned`.

**`В .env не заполнено TG_BOT_TOKEN` (или другое имя)**
Открой `1-настройки.cmd` и заполни строку. Значение пишется сразу после `=`,
без кавычек и без пробела.

**Окно моста мигнуло и закрылось**
Запускай именно `3-запустить-мост.cmd` — он держит окно открытым и показывает
ошибку. Если мигает и он, значит `.venv` не создан: запусти `0-установка.cmd`.

**`ModuleNotFoundError: No module named 'pymax'` (или `aiogram`)**
Библиотеки не поставились. Запусти `0-установка.cmd` ещё раз и посмотри, не
...
Подожди пару минут и запусти `2-вход-в-MAX.cmd` снова. Проверь, что номер
