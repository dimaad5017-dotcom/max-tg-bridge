...
- **Голосового и кружка в родном виде** — они доезжают, но выглядят в MAX иначе:
  голосовое приходит файлом `voice.ogg`, кружок — обычным видео. Оба открываются
  и слушаются, просто без круглой рамки и волны. Так вышло не от лени: голосовое
  MAX не принимает вовсе — отвечает «формат не поддерживается» на любой из пяти
  проверенных, — а кружок отправляется раньше, чем MAX успевает его дожать. Когда
  починят на той стороне — вернём.
