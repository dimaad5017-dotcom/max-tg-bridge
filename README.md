...
Минута работы, один раз:

1. Нажми `Win+R`, введи `shell:startup`, `Enter` — откроется папка автозагрузки.
2. Перетащи в неё `4-запустить-мост.cmd` **правой** кнопкой мыши и отпусти.
3. В выпавшем меню выбери **«Создать ярлыки»**.
4. Щёлкни по новому ярлыку правой кнопкой → «Свойства» → «Окно: свёрнутое в
   значок» → «ОК». Тогда мост при входе не будет разворачиваться на весь экран.
