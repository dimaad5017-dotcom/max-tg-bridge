...
**Как понять, что он живой:** напиши в группе `/help`. Ответил и ни на что не
жалуется — всё работает. Если сверху ответа стоит «⚠️ Связи с MAX нет», значит
работает только половина: команды мост слышит, а в MAX ничего не уходит и оттуда
ничего не приходит. Обычно он восстанавливает связь сам за минуту.

В окне при этом должны быть строки `Telegram на связи` и `MAX подключён`.

В окне мост пишет только своё и по-русски; полная запись всего, что происходит,
лежит в файле `cache/bridge.log`. Понадобилось полистать или скопировать —
двойной клик по **`посмотреть-лог.cmd`**, лог откроется Блокнотом.

### Автозапуск вместе с Windows — сделай сразу

Это выглядит как необязательная мелочь, но это самый важный шаг после запуска.
Мост работает, только пока открыто его окно. Выключил компьютер вечером, утром
не вспомнил открыть — и весь день сообщения копятся в MAX, куда ты как раз и не
заходишь. Узнаешь об этом не из Telegram, а от классного руководителя.

Двойной клик по **`5-автозапуск.cmd`**. Он покажет, включён автозапуск сейчас
или нет, и спросит, менять ли. Ответь `д` и нажми `Enter`.

Тем же файлом автозапуск и выключается — запусти ещё раз, он предложит убрать.

Проверить, что получилось: перезагрузи компьютер. Через полминуты напиши в
группе `/help` — должен ответить.

Что при этом происходит: в папку автозагрузки Windows кладётся один маленький
файл, который заводит мост свёрнутым в значок. Ту же папку можно открыть и
руками — `Win+R`, `shell:startup`, `Enter`, — там он и лежит, называется
`мост-MAX-Telegram.cmd`. Удалить его — то же самое, что выключить автозапуск.

Бояться, что мост запустится дважды, не нужно: второй копии он не даст
подняться и прямо об этом скажет. Так что двойной клик по
`4-запустить-мост.cmd` при уже работающем автозапуске ничего не сломает.
...
Если ставил через `git clone` — обновляться отдельно не нужно: `4-запустить-мост.cmd`
(и `mac-linux/4-run.sh`) сами делают `git pull` перед каждым запуском, и если
на GitHub вышла новая версия, мост подтянет её молча, без лишних вопросов.
Не вышло обновиться (нет интернета, есть свои правки поверх) — мост не
остановится, а просто запустится с тем, что уже есть.

Если ставил через **Code → Download ZIP** — так же, автоматически, не
получится: у ZIP-копии нет папки `.git`, и обновляться придётся руками.
