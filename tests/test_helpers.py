...
class TestСчётПоТелеграму:
    ...
    Название школьного чата — «9Б Дети 👶👶👶🧒🧒🧒». Такой текст Python меряет вдвое короче,
    ...
    def test_обрезка_названия_темы_считает_по_телеграму(self):
        ...
        название = "9Б Дети " + "👶" * 100

        assert _tg_len(название[: _cut(название, 128)]) <= 128


@pytest.fixture
def метки(monkeypatch, tmp_path):
    """Настоящее хранилище, но во временном файле: цвета мост запоминает, а не считает."""
...
class TestМеткиЛюдей:
    ...
    def test_один_и_тот_же_человек_всегда_одного_цвета(self, метки):
        """Иначе цвет ничего не значит: он должен пережить и перезапуск, и смену имени."""
        assert _mark(self.ЧАТ, 386174042) == _mark(self.ЧАТ, 386174042)

    def test_в_школьном_чате_у_всех_цвета_разные(self, метки):
        """Ради этого цвет и хранится: считанный из номера, он совпадал бы у половины класса.

        В «9Б Дети» девятнадцать человек, и цветов на них хватает с запасом — но только
        если раздавать по одному. Номера тут взяты вразнобой нарочно: важно, что цвета
        не зависят от того, какие они, — свободный найдётся любому, пока набор не кончился.
        """
        класс = [
            110482936, 152600741, 187345029, 219087563, 240915338,
            118273645, 204918377, 255300194, 301447761, 318209455,
            342771608, 355019284, 361884930, 372560117, 379113442,
            383992065, 388470513, 391225806, 394008271,
        ]  # fmt: skip

        assert len({_mark(self.ЧАТ, номер) for номер in класс}) == len(класс)

    def test_цвета_кончились_повтор_достаётся_самому_редкому(self, метки):
        """Народу больше, чем цветов, — повтор неизбежен, но пусть он будет ровно один."""
        толпа = [_mark(self.ЧАТ, 1000 + шаг) for шаг in range(len(NAME_MARKS) + 1)]
        сколько_раз = [толпа.count(метка) for метка in set(толпа)]

        assert sorted(сколько_раз) == [1] * (len(NAME_MARKS) - 1) + [2]

    def test_в_разных_чатах_цвета_считаются_отдельно(self, метки):
        """Иначе первый же чат съел бы всю палитру, и во втором всем достался бы повтор."""
        _mark(self.ЧАТ, 111)

        assert _mark(555, 222) == _mark(self.ЧАТ, 111)

    def test_метка_всегда_из_известного_набора(self, метки):
        assert all(_mark(self.ЧАТ, номер) in NAME_MARKS for номер in range(200))

    def test_без_отправителя_метка_ничья(self, метки):
        """Имени нет — цвет присвоить некому, но строка всё равно должна выглядеть ровно."""
        assert _mark(self.ЧАТ, None) == "🔘"
    ...
    def test_в_группе_метка_стоит_перед_именем(self, метки, monkeypatch):
        ...
        assert строка.startswith(f"{_mark(500, 386174042)} <b>Александра Н</b>\n")

    def test_в_личке_метки_нет(self, метки, monkeypatch):
