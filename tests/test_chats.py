...
def чат(chat_id=777, kind="CHAT", status="ACTIVE", title="9А класс", непрочитанных=0):
...
class ФальшивыйMAX:
    """Отдаёт историю страницами и от указанной точки — как настоящий, иначе листать нечего."""

    def __init__(self, чаты, история=()):
        self._чаты = чаты
        self._история = list(история)
    ...
    async def fetch_history(self, chat_id, backward, from_time=None):
        self.страниц += 1
        свежие_сначала = sorted(self._история, key=lambda m: m.time, reverse=True)
        if from_time is not None:
            свежие_сначала = [m for m in свежие_сначала if m.time < from_time]
        return свежие_сначала[:backward]
...
# Время MAX присылает в миллисекундах, и догонялка на это опирается: страницу истории
# она просит именно в них. На игрушечных единицах проверка ничего бы не проверила.
НАЧАЛО = 1_780_000_000_000
МИНУТА = 60_000


def сообщение(номер, отправитель=42):
    return SimpleNamespace(id=str(номер), time=НАЧАЛО + номер * МИНУТА, sender=отправитель)


@pytest.fixture
def догнанные(monkeypatch):
    """Что догонялка решила доставить. Саму доставку подменяем: проверяем отбор, а не отправку."""
    собрано = []

    async def доставить(chat_id, message):
        собрано.append(message)

    monkeypatch.setattr(main, "_try_deliver", доставить)
    return собрано


class TestГлубинаДогонялки:
    """Мост брал у MAX одну страницу истории — сорок сообщений, — и на этом догон кончался.

    Пока он стоял час, хватало с избытком. Но за неделю простоя в школьном чате
    набирается полторы сотни: сорок доезжали, сто десять исчезали. Беззвучно, что здесь
    и есть главная беда: метка «докуда доставлено» после догона прыгала в самый конец,
    и пропавшие сто десять не всплывали уже никогда — ни в этот запуск, ни в следующий.
    """

    def догнать(self, история, доставлено=None, непрочитанных=0):
        if доставлено is not None:
            main.topics.remember_delivered(777, доставлено)
        макс = ФальшивыйMAX([чат(непрочитанных=непрочитанных)], история)
        asyncio.run(main._catch_up(макс))
        return макс

    def test_догоняет_дальше_первой_страницы(self, мост, догнанные):
        """Ради этого всё и затевалось: сто сообщений — это две с половиной страницы MAX."""
        сто = [сообщение(номер) for номер in range(1, 101)]

        self.догнать(сто, доставлено=НАЧАЛО)

        assert len(догнанные) == 100

    def test_листает_а_не_просит_всё_разом(self, мост, догнанные):
        """Проверяем именно листание: одним запросом MAX столько и не отдаст."""
        макс = self.догнать([сообщение(номер) for номер in range(1, 101)], доставлено=НАЧАЛО)

        assert макс.страниц > 1

    def test_останавливается_на_уже_доставленном(self, мост, догнанные):
        """Иначе каждый запуск моста заново вываливал бы в тему всю переписку."""
        сто = [сообщение(номер) for номер in range(1, 101)]

        self.догнать(сто, доставлено=НАЧАЛО + 50 * МИНУТА)

        assert [m.id for m in догнанные] == [str(номер) for номер in range(51, 101)]

    def test_свои_сообщения_не_догоняет(self, мост, догнанные):
        """Написанное с телефона в MAX в теме уже есть — вернётся эхом."""
        свои = [сообщение(номер, отправитель=1) for номер in range(1, 101)]

        self.догнать(свои, доставлено=НАЧАЛО)

        assert догнанные == []

    def test_упёршись_в_потолок_говорит_об_этом(self, мост, догнанные, monkeypatch):
        """Молча обрезать нельзя: в MAX ты не заходишь и о пропаже не узнаешь никогда."""
        monkeypatch.setattr(main, "HISTORY_LIMIT", 20)

        self.догнать([сообщение(номер) for номер in range(1, 101)], доставлено=НАЧАЛО)

        assert any("осталось только в MAX" in текст for текст in мост.отправлено)

    def test_из_потолка_оставляет_свежее(self, мост, догнанные, monkeypatch):
        """Сегодняшнее «заберите ребёнка» важнее того, что писали неделю назад."""
        monkeypatch.setattr(main, "HISTORY_LIMIT", 20)

        self.догнать([сообщение(номер) for номер in range(1, 101)], доставлено=НАЧАЛО)

        assert [m.id for m in догнанные] == [str(номер) for номер in range(81, 101)]

    def test_уложившись_в_потолок_лишнего_не_говорит(self, мост, догнанные):
        """Предупреждение на каждом запуске человек перестанет читать — и пропустит настоящее."""
        self.догнать([сообщение(номер) for номер in range(1, 11)], доставлено=НАЧАЛО)

        assert not any("осталось только в MAX" in текст for текст in мост.отправлено)

    def test_пустая_история_не_уводит_в_вечный_круг(self, мост, догнанные):
        макс = self.догнать([], доставлено=НАЧАЛО)

        assert макс.страниц == 1
        assert догнанные == []

    def test_если_max_отдаёт_одно_и_то_же_догонялка_не_виснет(self, мост, догнанные):
        """Мост стоит на чужом сервере: тот вправе не понять «от какого места» и слать первое.

        Без этой страховки догонялка листала бы одну и ту же страницу до самого потолка,
        а с ним — навсегда: живой с виду мост, который не доставляет ничего.
        """

        class Заело(ФальшивыйMAX):
            async def fetch_history(self, chat_id, backward, from_time=None):
                return await super().fetch_history(chat_id, backward)

        main.topics.remember_delivered(777, НАЧАЛО)
        макс = Заело([чат()], [сообщение(номер) for номер in range(1, 101)])

        asyncio.run(asyncio.wait_for(main._catch_up(макс), timeout=5))

        assert макс.страниц == 2
