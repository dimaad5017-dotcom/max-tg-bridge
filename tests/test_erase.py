...
class TestРеакцияНаНезнакомоеСообщение:
    """Мост не знает это сообщение — послать реакцию некуда. Молчать об этом нельзя.

    31 августа мост принял реакцию и не сделал ничего: в связке такого сообщения не было,
    обработчик вышел на первой же проверке. Ни строки в теме, ни строки в логе — по логу
    удача и такой выход выглядели одинаково, то есть никак. Реакция при этом стоит под
    сообщением и выглядит отправленной ровно как настоящая.
    """

    def test_говорит_что_реакция_никуда_не_ушла(self, мост):
        бот, макс = мост()

        asyncio.run(main.on_tg_reaction(реакция("❤", message_id=999)))

        assert макс.запросы == [], "послать было некуда"
        assert бот.сказано, "промолчал — значит, человек уверен, что реакция уехала"
        assert "никуда не ушла" in бот.сказано[0]

    def test_про_корзину_говорит_что_стирать_нечего(self, мост):
        """Худший случай: уйдёшь уверенным, что стёр у собеседника, а оно на месте."""
        бот, макс = мост()

        asyncio.run(main.on_tg_reaction(реакция(main.DELETE_MARK, message_id=999)))

        assert макс.стёрто == []
        assert "Стирать в MAX тоже нечего" in бот.сказано[0]

    def test_снятие_не_беспокоит(self, мост):
        """Убрать свою же реакцию — уборка, а не распоряжение. Отвечать не на что."""
        бот, _ = мост()

        asyncio.run(main.on_tg_reaction(реакция(None, message_id=999)))

        assert бот.сказано == []

    def test_знакомое_сообщение_по_прежнему_уезжает(self, мост):
        """Страховка от слишком широкой сетки: обычная реакция не должна сюда попадать."""
        бот, макс = мост()

        asyncio.run(main.on_tg_reaction(реакция("❤")))

        assert len(макс.запросы) == 1
        assert бот.сказано == []
